...
function WorkflowDiagram() {
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-2">
        {steps.map((step, index) => (
          <div key={step.label} className="flex items-center gap-2 sm:gap-0 w-full sm:w-auto">
            ...
            >
              <div className="w-14 h-14 rounded-xl bg-foreground/5 border border-border flex items-center justify-center mb-2 group-hover:bg-foreground/10 transition-colors">
      ...
      <motion.div
        className="hidden sm:block absolute top-1/2 left-[15%] right-[15%] h-px bg-gradient-to-r from-border via-foreground/20 to-border -translate-y-1/2 -z-10"
        initial={{ scaleX: 0 }}
        ...
        <div className="mb-16 lg:mb-20">
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-end">
            ...
            >
              Useful systems.
              <br />
              <span className="text-muted-foreground/60">No gimmicks.</span>
            </motion.h2>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
          ...
          {aiCapabilities.map((capability, index) => (
            ...
            >
              <div className="w-10 h-10 rounded-lg border border-border bg-background flex items-center justify-center mb-4 group-hover:border-foreground/20 group-hover:bg-foreground/5 transition-all">
                <capability.icon className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors" />
              </div>
