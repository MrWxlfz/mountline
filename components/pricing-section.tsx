...
export function PricingSection() {
  ...
  return (
    <section id="pricing" className="py-28 lg:py-36 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 lg:mb-20">
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-end">
            ...
            >
              Simple packages.
              <span className="text-foreground/35"> Clear scope.</span>
            </motion.h2>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className={`h-full rounded-2xl border transition-all duration-300 ${
                plan.highlight 
                  ...
                  : "border-foreground/[0.06] bg-card hover:border-foreground/12 hover:shadow-xl hover:shadow-foreground/[0.03]"
              }`}>
                {/* Badge */}
                {plan.badge && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="inline-flex items-center px-3.5 py-1.5 rounded-lg bg-background text-foreground text-[10px] font-medium uppercase tracking-wider shadow-lg border border-foreground/10">
                      {plan.badge}
                    </span>
                  </div>
                )}
                ...
                <div className="p-7">
                  {/* Plan name */}
                  <h3 className={`text-[11px] font-medium uppercase tracking-wider mb-6 ${
                  ...
                  <div className="mb-5">
                    ...
                    <div className={`text-[10px] uppercase tracking-wider mt-1.5 ${
                      plan.highlight ? "text-background/40" : "text-foreground/30"
                    }`}>
                      {plan.priceNote}
                    </div>
                  </div>
                  ...
                  <p className={`text-sm leading-relaxed mb-6 ${
                    plan.highlight ? "text-background/65" : "text-foreground/50"
                  }`}>
                    {plan.description}
                  </p>
                      ...
                      <li key={feature} className="flex items-start gap-3">
                        <div className={`w-5 h-5 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                          ...
                          <Check className={`w-3 h-3 ${
                            plan.highlight ? "text-background" : "text-foreground/55"
                          }`} />
                        </div>
                  ...
                  <motion.button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-sm font-medium transition-all ${
                      plan.highlight
                        ...
                        : "bg-foreground/[0.03] hover:bg-foreground/[0.06] text-foreground border border-foreground/[0.06] hover:border-foreground/12"
                    }`}
                  >
                    Get started
        ...
        <motion.div
          ...
          className="mt-14 flex items-center justify-center gap-3 text-sm text-foreground/45"
        >
        ...
        <motion.p
          ...
          className="text-center text-sm text-foreground/30 mt-5"
        >
